...
connector = sqlite3.connect('ClientData.db')
cursor = connector.cursor()

connector.execute(
"CREATE TABLE IF NOT EXISTS CLIENT_MANAGEMENT (CLIENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, CITY TEXT)"
)

# Creating the functions
def reset_fields():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, city_strvr

    for i in ['name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'city_strvr']:
        exec(f"{i}.set('')")
...
def display_records():
    ...
    curr = connector.execute('SELECT * FROM CLIENT_MANAGEMENT')
    data = curr.fetchall()
...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not city:
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
            'INSERT INTO CLIENT_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, CITY) VALUES (?,?,?,?,?,?)', (name, email, contact, gender, DOB, city)
            )
            connector.commit()
...
def remove_record():
    if not tree.selection():
        ...
        connector.execute('DELETE FROM CLIENT_MANAGEMENT WHERE CLIENT_ID=%d' % selection[0])
        connector.commit()
...
def view_record():
    ...
    dob.set_date(date); city_strvr.set(selection[6])


# Initializing the GUI window
main = Tk()
main.title('Client Management Tool')
main.geometry('1000x600')
main.resizable(0, 0)

# Creating the background and foreground color variables
lf_bg = 'gray90' # bg color for the left_frame
cf_bg = 'gray90' # bg color for the center_frame
...
city_strvr = StringVar()

# Placing the components in the main window
Label(main, text="Client Management Tool", font=headlabelfont, bg='cornsilk1').pack(side=TOP, fill=X)
...
Entry(left_frame, width=19, textvariable=city_strvr, font=entryfont).place(x=20, rely=0.75)
...
Label(right_frame, text='Clients Records', font=headlabelfont, bg='black', fg='white').pack(side=TOP, fill=X)

tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                    columns=('Client ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "City"))
